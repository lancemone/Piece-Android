...
object StatusBarUtil {
    ...
    const val TAG_KEY_HAVE_SET_OFFSET = -123
    val FAKE_STATUS_BAR_VIEW_ID = R.id.statue_bar_fake_view
    val FAKE_TRANSLUCENT_VIEW_ID = R.id.statue_bar_translucent_view
    ...
    fun setColor(activity: Activity, @ColorInt color: Int, @IntRange(from = 0, to = 255) statusBarAlpha: Int){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            // 将FLAG_TRANSLUCENT_STATUS这个windowFlag换成FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS，否则状态栏不会完全透明，会有一个半透明的灰色蒙层
